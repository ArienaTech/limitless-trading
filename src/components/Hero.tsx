      ...
      {words.map((word, wi) => (
        <span key={wi} style={{ display: "inline-block", overflow: "hidden" }}>
          <motion.span
            ...
            initial={{ y: "110%" }}
            animate={{ y: 0 }}
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.2 + wi * 0.1 }}
          >
            {word}
          </motion.span>
        </span>
    ...
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center gutter">
        ...
        <motion.div {...fade(0.1)} className="flex items-center gap-2 mb-8">
          ...
          <span className="mono text-[10px] text-text-soft tracking-[0.15em]">Q3 2026 COHORT — LIMITED SPOTS</span>
        </motion.div>
        ...
        {/* Subline */}
        <motion.p {...fade(0.5)} className="text-text-soft text-[16px] mt-6 max-w-lg leading-relaxed">
          Systematic forex trading for serious investors. FCA regulated.
          Founded in London, 2014.
        </motion.p>

        {/* CTAs */}
        <motion.div {...fade(0.62)} className="mt-10 flex flex-col sm:flex-row items-center gap-4">
          <motion.a
            href="#apply"
        ...
        {/* Trust micro-copy */}
        <motion.p {...fade(0.72)} className="mono text-[9px] text-text-dim mt-5 tracking-[0.1em]">
